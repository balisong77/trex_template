...
class STLS1(object):
    ...
    def create_stream(self, dir, port_id):
        ...
        stream = [
            ...
            # ),
            # STLStream(
            #     packet=STLPktBuilder(pkt=add_padding(change_ip4_tos(pkt4_udp, 2), 1500), vm=vm4),
            #     mode=STLTXCont(pps=ip4_pps),
            ...
            # ),
            STLStream(
                packet=STLPktBuilder(pkt=add_padding(change_ip4_tos(pkt4_udp, 1), 1500), vm=vm4_nat),
                mode=STLTXCont(pps=ip4_pps),
            ...
            # ),
            # STLStream(
            #     packet=STLPktBuilder(pkt=add_padding(change_ip4_tos(pkt4_tcp, 6), 1500), vm=vm4),
            #     mode=STLTXCont(pps=ip4_pps),
            ...
            # ),
            # STLStream(
            #     packet=STLPktBuilder(pkt=add_padding(change_ip4_tos(pkt4_tcp, 8), 1500), vm=vm4_nat),
            #     mode=STLTXCont(pps=ip4_pps),
            ...
            # ),
            # STLStream(
            #     packet=STLPktBuilder(pkt=add_padding(change_ip6_tc(pkt6_udp, 10), 1500), vm=vm6),
            #     mode=STLTXCont(pps=ip6_pps),
            #     flow_stats=STLFlowStats(pg_id=10),
            # ),
            # IPv6 TCP
            STLStream(
                packet=STLPktBuilder(pkt=add_padding(change_ip6_tc(pkt6_tcp, 2), 66), vm=vm6),
                mode=STLTXCont(pps=ip6_pps),
                flow_stats=STLFlowStats(pg_id=11),
            ),
            # STLStream(
            #     packet=STLPktBuilder(pkt=add_padding(change_ip6_tc(pkt6_tcp, 12), 1500), vm=vm6),
            #     mode=STLTXCont(pps=ip6_pps),
