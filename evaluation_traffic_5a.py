    ...
    def create_stream(self, dir, port_id):
        ...
        protocol_flow_stream = [
            STLStream(
                ...
                packet=STLPktBuilder(pkt=add_padding(change_ip6_protocol_num(change_ip6_tc(pkt6_udp_protocol, 9), i), self.small_packet_padding), vm=no_vm),
                mode=STLTXCont(pps=pareto_dict[self.workload_num][self.pareto_alpha][i]),
                flow_stats=STLFlowStats(pg_id=1),
            )
            for i in range(8, self.workload_num)
        ]

        return big_flow_stream + small_flow_stream + protocol_flow_stream
